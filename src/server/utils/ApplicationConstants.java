...
public interface ApplicationConstants {
		...
		public final String GET_ALL_APPROVED_REVIEWS = "SELECT r.uid AS uid, r.bid AS bid, c.nickname AS nickname, b.name AS book_name, r.approved_by AS approved_by, r.text as text FROM reviews r INNER JOIN books b ON r.bid=b.bid INNER JOIN customers c ON r.bid=c.bid ORDER BY r.bid WHERE r.approved_by IS NOT NULL";
		public final String GET_ALL_UNAPPROVED_REVIEWS = "SELECT r.uid AS uid, r.bid AS bid, c.nickname AS nickname, b.name AS book_name, r.approved_by AS approved_by, r.text as text FROM reviews r INNER JOIN books b ON r.bid=b.bid INNER JOIN customers c ON r.bid=c.bid ORDER BY r.bid WHERE r.approved_by IS NULL"; 
		...
		public final String GET_REVIEWS_FOR_BOOK ="SELECT r.uid AS uid, r.bid AS bid, c.nickname AS nickname, b.name AS book_name, r.approved_by AS approved_by, r.text as text FROM reviews r INNER JOIN books b ON r.bid=b.bid INNER JOIN customers c ON r.bid=c.bid WHERE r.bid=? AND r.approved_by IS NOT NULL";
